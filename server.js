...
app.post('/offers', auth, requireRole('comite_ajout'), uploadTdr.single('tdr'), async (req, res) => {
  try {
    const { type, title, description, country, project_id, reference, deadline, notification_emails, custom_documents, removed_default_documents } = req.body;
    ...
    const [result] = await pool.query(
      `INSERT INTO offers (type, title, description, country, project_id, reference, deadline, status, created_by, tdr_filename, tdr_filepath, notification_emails, removed_default_documents)
       VALUES (?, ?, ?, ?, ?, ?, ?, 'actif', ?, ?, ?, ?, ?)`,
      [type, title, description, country, project_id, reference, deadline, req.user.id, tdrFilename, tdrFilepath, JSON.stringify(emails), JSON.stringify(removedDefaultDocs)]
    );
